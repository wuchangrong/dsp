...
<script language="javascript" src="<%=ctx%>/platform/jsp/sm/privilege/user/UserProhibitList.js"></script>
...
<%
    ...
    //TODO: 这一段为"/include/searchBody.jsp" 设置参数 fieldList.
...
<html:form action="/sm/privilege/user/UserProhibit.do">
<html:hidden property="method" value="doSearch"/>
<html:hidden property="doWhat" />
<input type="hidden" name="whereToBack" value="userProhibit">
<input type="hidden" name="doSave" value="<%=request.getAttribute("doSave")%>"/>
<input type="hidden" name="programId" value="<%=request.getParameter("programId")%>"/>
	
<handao:headLink titleKey="common.sm.privilege.user.prohibit.title" bundle="<%=bundleKey%>">
    <handao:headButton name="查询" accesskey="Q" onClick="queryRecord();"/>
    <handao:headButton name="查看明细" accesskey="V" onClick="viewRecord();"/>
   	<handao:headButton name="启用/停用" accesskey="C" onClick="usingChange();"/>
<%--   	<handao:headButton name="帮助" accesskey="H" onClick="helpClick()"/>--%>
</handao:headLink>
