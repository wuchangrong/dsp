...
${id} = new UniEXT.lookup.GridLookupField({
    hiddenID:'${row_id_pro}',
    applyTo:'${name_pro}',
    sqlID:'common.lookup.gridlookup.employee',
    winTitle:'<bean:message key="common.lookup.employee.title" bundle="<%=bundleKey%>" />',
    pageSize:${pagesize==null?15:pagesize},
    singleSelect:${singleselect==null?'true':singleselect},
    columnModel:[
        {header:'ROW_ID',dataIndex:'ROW_ID',hidden:true},
        {header:'<bean:message key="common.lookup.employee.code" bundle="<%=bundleKey%>" />',dataIndex:'CODE',width:60,sortable:true},
        {header:'<bean:message key="common.lookup.employee.name" bundle="<%=bundleKey%>" />',dataIndex:'NAME',width:60,sortable:true}
    ],
    storeCfg:{
